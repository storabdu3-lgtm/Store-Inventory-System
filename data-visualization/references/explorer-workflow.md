...
await screenshot({ source: { type: "appPreview", artifactDirName: "my-explorer", path: "/" } });
```
...
Same as dashboard workflow -- use the `AskQuestion` model tool to offer a deeper analysis as a markdown file. See `references/detailed-analysis.md`.

After this step, call `SuggestUserAction({ action: "deploy", message: "The explorer is ready to publish." })`.

## Explorer Subagent Task Template

**NOTE: Do not update App.tsx with the explorer component until the design completion notification arrives.** This will ensure that the user doesn't see a broken state.

Run CodeExecution with:

```js
const explorerDesignTask = `Build a dataset explorer.
