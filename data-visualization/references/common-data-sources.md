...
1. Search: `searchIntegrations({ mode: "search", queries: ["stripe"] })`
2. Check status: `"added"`, `"not_added"`, `"not_setup"`, or `"requires_setup"`
...
// 3. Launch parallel subagents to explore each group (`kind: "general"`)
...
const exploreTablesTask = `Explore these BigQuery tables: analytics.events, analytics.sessions...
  For each:
...
const exploreTablesSubagentJob = subagent({
  name: "explore-tables",
  task: exploreTablesTask,
  config: { $kind: "general" },
});
// const exploreOtherGroupJob = subagent(...);
const groupResults = await Promise.all([exploreTablesSubagentJob /* ...other group jobs... */]);

// 4. Wait for all exploration, then createArtifact()
```

**Key points:** Happens BEFORE `createArtifact()`. Keep to 2-4 general subagents. Skip for schemas with <15 tables.
