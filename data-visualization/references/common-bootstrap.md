...
2. Use `searchIntegrations` and the status-driven integration workflow below to connect data sources
...
1. **User mentioned specific sources** (external databases, APIs, services):
   - `searchIntegrations({ mode: "search", queries: ["Linear"] })`
   - Check status: `"added"`, `"not_added"`, `"not_setup"`, or `"requires_setup"`
   - Use the exact `id` returned by `searchIntegrations`:
     - `not_added`: `addIntegration({ integrationId: result.id })`
     - `not_setup` or `requires_setup`: `ProposeIntegration({ proposal: [{ integrationId: result.id }] })` (`requires_setup` IDs use `connector_catalog:<name>`)
...
2. **No data source specified**: If the data visualization artifact setup question already asked for sources, use its answer. Otherwise, use `searchIntegrations({ mode: "list" })` to list available connectors and suggest them to the user.
...
const result = await createArtifact({
    ...
    title: "My Dashboard",
    description: "An interactive dashboard charting monthly sales and top products from the uploaded CSV. Includes trend lines, category breakdowns, and a filterable data table."
});
```
...
1. Start the design subagent through CodeExecution and do not await it
2. While it runs, implement api-server routes in `artifacts/api-server/src/routes/`
3. Continue independent work and handle the design completion notification when it arrives
...
```js
const dataVizDesignTask = `Build the data visualization [DASHBOARD|REPORT|EXPLORER]...`;
subagent({
  name: "data-viz-design",
  task: dataVizDesignTask,
  config: {
    $kind: "design",
    relevantFiles: [
...
3. **Screenshot** only after curl confirms APIs work. Sleep for 15s after making a web request to ensure that the data is fully loaded before taking the screenshot.
  ...
  ```javascript
  await screenshot({ source: { type: "appPreview", artifactDirName: "my-dashboard", path: "/" } });
  ```
