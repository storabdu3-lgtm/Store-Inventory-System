...
After delivering the brief analysis summary, ask the user if they want a deeper analysis. Use the `AskQuestion` model tool (NOT plain text) to present the question.

After delivering the brief analysis summary in the previous step, immediately call the `AskQuestion` model tool:

```json
{
  "question": "Would you like me to generate a detailed analysis report with findings and recommendations based on this data?",
  "choices": ["Yes, generate a detailed report", "No, the summary is enough"]
}
```

**You are required to use the `AskQuestion` model tool for this step — do NOT ask as plain text in chat.** The tool renders an interactive question, which is a better experience than a plain text question the user has to type a response to.
...
After this step, call `SuggestUserAction({ action: "deploy", message: "The dashboard is ready to publish." })`.
...
**NOTE: Do not update App.tsx with the dashboard component until the design completion notification arrives.** This will ensure that the user doesn't see a broken state.

Run CodeExecution with:

```js
const dashboardDesignTask = `Build the data visualization dashboard.
