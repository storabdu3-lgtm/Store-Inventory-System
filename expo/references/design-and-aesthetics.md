...
When building a mobile artifact in a multi-artifact project, synchronize the design tokens before writing component code. The mobile app should share the same visual identity — colors, fonts, and border radius — so all artifacts feel like one product.

Inspect the current Artifacts state before choosing a source. If a `design-system` artifact exists now, use the one selected through the Expo skill as the source of truth and follow its `docs/AGENTS.md` Expo consumption instructions. Do not derive a parallel theme from a sibling web artifact.

Use the sibling-web-artifact fallback below only when the current Artifacts state has no design system.

### Colors

Derive the mobile palette from the sibling web artifact rather than inventing a new one. Steps:
...
- Always use the semantic theme returned by `useColors()` — import it directly from the selected design system when one exists, and never hardcode hex values in components.
...
Before using the example, import `useColors` from the exact package path in the
selected design system's `docs/AGENTS.md`. When no design system exists, import
it from the Expo scaffold's `@/hooks/useColors` path.
