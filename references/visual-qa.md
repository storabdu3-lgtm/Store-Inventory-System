...
- **`overflow: scroll` to "hide" text that overflows the slide frame.** Slides don't scroll. If body text overflows, fix the content. Internal scroll on table/chart containers (per the `<constraints>` allowed-interactive list in `building.md`) is fine — the rule is no scroll on the *slide* itself.
...
Before telling the user the deck is ready, verify `src/App.tsx` still matches the platform contract. Most export-related violations below silently time out with `NO_SLIDES_FOUND` after 60 seconds even if the preview looks fine; the `data-slide-id` violation affects thumbnail-to-slide pairing instead.
...
- The `data-slide-id={slide.id}` attribute on the `AllSlides` wrapper has been removed or stamped with something other than the slide's manifest `id` (this one degrades thumbnail-to-slide pairing rather than exports — a missing attribute forces fragile position-order matching, while an incorrect non-empty id can skip or mis-pair the capture).
...
- **`data-slide-id` removed or mis-stamped** on the `.slide` wrapper → restore `data-slide-id={slide.id}` using the manifest `id` of the wrapped slide.
