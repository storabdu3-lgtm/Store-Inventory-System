...
The codegen script already runs `pnpm -w run typecheck:libs`. If you change `lib/*` again afterward, rerun `pnpm run typecheck:libs` before checking artifact packages so leaf typechecks see fresh workspace declarations.
